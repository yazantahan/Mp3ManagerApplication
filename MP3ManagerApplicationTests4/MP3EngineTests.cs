...
namespace MP3ManagerApplication.Tests
{
    ...
    public class MP3EngineTests
    {
        ...
        public void getMP3ArtistFilesTest()
        {
            ...
            string mp3ListArtists = mp3Engine.getMP3ArtistFiles();

            Console.WriteLine(mp3ListArtists);
            Assert.IsTrue(string.IsNullOrEmpty(mp3ListArtists));
        ...
        public void initYearListTest()
        {
            ...
            Dictionary<int, string> mp3Artists_List = mp3Engine.initArtistList();

            foreach (var mp3Artist in mp3Artists_List)
            {
                Console.WriteLine(mp3Artist.Key + "- " + mp3Artist.Value);
            }

            Assert.AreEqual(20, mp3Artists_List.Count);
        }
    }
}